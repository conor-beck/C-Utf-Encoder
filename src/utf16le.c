...
//#endif
  ...
  while ((bytes_read2 = read_to_bigendian(infile, &buf->upper_bytes, 2)) > 0) {
    if(bytes_read2 < 2){
  ...
  while((bytes_read = read_to_bigendian(infile, &utf16_buf->upper_bytes, 2)) > 0){
    ...
    if((utf16_buf->upper_bytes >= 0xD800) && (utf16_buf->upper_bytes <= 0xDBFF)){
      ...
      if(code_point <= 0x7F){
        ...
        bytes_read2 = write_to_bigendian(outfile, &ret.bytes[0].byte, 1);
        if(bytes_read2 < 1){
        ...
        bytes_read2 = write_to_bigendian(outfile, &ret.bytes[1].byte, 1);
        if(bytes_read2 < 1){
          printf("Couldn't write to file\n");
          exit(EXIT_FAILURE);
        }
        ...
        bytes_read2 = write_to_bigendian(outfile, &ret.bytes[2].byte, 1);
        if(bytes_read2 < 1){
          printf("Couldn't write to file\n");
          exit(EXIT_FAILURE);
        }
        ...
        bytes_read2 = write_to_bigendian(outfile, &ret.bytes[3].byte, 1);
        if(bytes_read2 < 1){
          printf("Couldn't write to file\n");
          exit(EXIT_FAILURE);
        }
...
utf16_glyph_t*
code_point_to_utf16le_glyph(code_point_t code_point, utf16_glyph_t* ptr)
{
  code_point_t temp = code_point;
  ptr->lower_bytes = 0;
  ptr->upper_bytes = 0;
  //utf16_glyph_t ret;
  //utf16_glyph_t* ptr;
  //ptr = malloc(sizeof ret);
  //memeset(ptr, 0, sizeof ret, 0);
  if(code_point >= 0x10000) {
    temp = code_point - 0x10000;
    ptr->upper_bytes = (temp >> 10) + 0xD800;
    //ptr->upper_bytes = reverse_bytes(ptr->upper_bytes);
    ptr->lower_bytes = (temp & 0x3FF) + 0xDC00;
